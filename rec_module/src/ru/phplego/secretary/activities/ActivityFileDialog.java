...
public class ActivityFileDialog extends ListActivity {
	...
	*/
	public void onCreate(Bundle savedInstanceState) {
		...
		if(newFileName != null){
				...
				setCreateVisible(newButton);
				mFileName.setText(newFileName);
				mFileName.requestFocus();
		}
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		if (file.isDirectory()) {
						...
						if (file.canRead()) {
										...
										new AlertDialog.Builder(this).setIcon(App.getIcon())
	...
	*/
	...
	public static void run(Activity activity, int request_code, String root, String [] filter) {
		if(root == null) root = Environment.getExternalStorageDirectory().getAbsolutePath();
		Intent intent = new Intent(activity.getBaseContext(), ActivityFileDialog.class);
		File fRoot = new File(root);
		if(!fRoot.isDirectory()){
				intent.putExtra(ActivityFileDialog.START_PATH, fRoot.getParent());
				intent.putExtra(ActivityFileDialog.FILE_NAME, fRoot.getName());
		}
		else
				intent.putExtra(ActivityFileDialog.START_PATH, root);

		//can user select directories or not
		//intent.putExtra(ActivityFileDialog.CAN_SELECT_DIR, true);

		//alternatively you can set file filter
		//intent.putExtra(ActivityFileDialog.FORMAT_FILTER, new String[] { "png" });
		if(filter != null) intent.putExtra(ActivityFileDialog.FORMAT_FILTER, filter);

		activity.startActivityForResult(intent, request_code);
	}

	public static void run(Activity activity, int request_code, String root) {
		run(activity, request_code, root, null);
	}

	public static void run(Activity activity, int request_code) {
		run(activity, request_code, null, null);
	}
